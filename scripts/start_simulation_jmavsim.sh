#!/bin/bash
# Start PX4 SITL with jMAVSim (lightweight, no Gazebo)
# Provides sensors and airframe without 3D visualization
# Perfect for Fixed-Wing and VTOL development without X11 issues
#
# Usage:
#   ./scripts/start_simulation_jmavsim.sh                    # Fixed-wing (default)
#   ./scripts/start_simulation_jmavsim.sh --vehicle vtol     # VTOL

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
COMPOSE_FILE="$PROJECT_DIR/docker/simulation/docker-compose.jmavsim.yml"
...
echo -e "${CYAN}================================================${NC}"
echo -e "${CYAN}  PX4 ${VEHICLE_NAME} Simulation (jMAVSim)${NC}"
echo -e "${CYAN}  Lightweight - No Gazebo Required${NC}"
echo -e "${CYAN}================================================${NC}"
echo ""
...
docker rm -f px4_jmavsim 2>/dev/null || true
sleep 2

# Start simulation
echo -e "${YELLOW}[2/5] Starting PX4 SITL with jMAVSim...${NC}"
...
while [ $WAITED -lt $MAX_WAIT ] && [ "$READY" = false ]; do
    ...
    if docker logs px4_jmavsim 2>&1 | grep -q "pxh>"; then
        READY=true
...
docker exec px4_jmavsim pip3 install mavproxy pymavlink --quiet 2>/dev/null || {
    docker exec px4_jmavsim bash -c "
...
docker exec -d px4_jmavsim bash -c "mavproxy.py --master=udp:127.0.0.1:14540 --out=udp:0.0.0.0:14550 --out=tcpin:0.0.0.0:5760 --daemon"
sleep 5

if docker exec px4_jmavsim bash -c "pgrep -f mavproxy > /dev/null" 2>/dev/null; then
...
docker exec px4_jmavsim python3 /scripts/fix_ekf_origin_startup.py ${PX4_HOME_LAT:-36.2329} ${PX4_HOME_LON:--116.8276} ${PX4_HOME_ALT:-50} 2>&1 | grep -E "(SUCCESS|ERROR)" || true
docker exec px4_jmavsim python3 /scripts/set_vehicle_position.py ${PX4_HOME_LAT:-36.2329} ${PX4_HOME_LON:--116.8276} ${PX4_HOME_ALT:-50} 2>&1 | grep -E "(SUCCESS|ERROR)" || true
...
echo "  ✓ Airframe configuration (Fixed-wing)"
echo "  ✓ Position estimates"
echo "  ✓ No 3D visualization (headless)"
echo ""
