#!/bin/bash
# Start PX4 SITL with Gazebo Harmonic Headless (Robust, No GUI)
# Gazebo without GUI - more reliable, no X11 issues
# Supports fixed-wing and VTOL with full physics
#
# Usage:
#   ./scripts/start_simulation_gazebo_headless.sh                    # Fixed-wing (default)
#   ./scripts/start_simulation_gazebo_headless.sh --vehicle vtol     # VTOL

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
COMPOSE_FILE="$PROJECT_DIR/docker/simulation/docker-compose.gazebo-headless.yml"
...
echo -e "${CYAN}================================================${NC}"
echo -e "${CYAN}  PX4 ${VEHICLE_NAME} Simulation (Gazebo Headless)${NC}"
echo -e "${CYAN}  Robust - Full Physics, No GUI${NC}"
echo -e "${CYAN}================================================${NC}"
echo ""
...
docker rm -f px4_gazebo_headless 2>/dev/null || true
sleep 2

# Start simulation
echo -e "${YELLOW}[2/5] Starting PX4 SITL with Gazebo (headless)...${NC}"
...
while [ $WAITED -lt $MAX_WAIT ] && [ "$READY" = false ]; do
    ...
    if docker logs px4_gazebo_headless 2>&1 | grep -q "pxh>"; then
        READY=true
...
if [ "$READY" = false ]; then
    ...
    docker logs px4_gazebo_headless 2>&1 | tail -20
fi

# Install and start MAVProxy
echo -e "${YELLOW}[4/5] Setting up MAVLink forwarding for QGroundControl...${NC}"
docker exec px4_gazebo_headless pip3 install mavproxy pymavlink --quiet 2>/dev/null || {
    docker exec px4_gazebo_headless bash -c "
...
docker exec -d px4_gazebo_headless bash -c "mavproxy.py --master=udp:127.0.0.1:14540 --out=udp:0.0.0.0:14550 --out=tcpin:0.0.0.0:5760 --daemon"
sleep 5

if docker exec px4_gazebo_headless bash -c "pgrep -f mavproxy > /dev/null" 2>/dev/null; then
...
docker exec px4_gazebo_headless python3 /scripts/fix_ekf_origin_startup.py ${PX4_HOME_LAT:-36.2329} ${PX4_HOME_LON:--116.8276} ${PX4_HOME_ALT:-50} 2>&1 | grep -E "(SUCCESS|ERROR)" || true
docker exec px4_gazebo_headless python3 /scripts/set_vehicle_position.py ${PX4_HOME_LAT:-36.2329} ${PX4_HOME_LON:--116.8276} ${PX4_HOME_ALT:-50} 2>&1 | grep -E "(SUCCESS|ERROR)" || true
...
echo -e "${GREEN}================================================${NC}"
echo ""
echo "Container: px4_gazebo_headless"
echo "Vehicle: ${VEHICLE_NAME} (${VEHICLE})"
echo "Airframe: SYS_AUTOSTART=${SYS_AUTOSTART}"
echo "Simulator: Gazebo Harmonic (headless, no GUI issues)"
echo ""
...
echo "  ✓ No X11/GUI required (headless mode)"
echo ""
